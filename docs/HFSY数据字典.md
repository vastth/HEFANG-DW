...
> 说明: 本文档基于 hfsy 实库只读结构快照生成，用于数云 CRM 接入阶段的真实字段核对；不包含账号密码等敏感连接信息。
...
- `t_member_bind_info` 已包含 `platAccount1`、`bindMobile1`、`birthday1`、`gender1`、`name1` 等解密后字段，优先级高于原密文字段。
- `t_order_copy` 与 `t_order_copy1` 当前仅记录结构和行数，尚未确认是否为正式业务表。
