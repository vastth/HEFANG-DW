...
### [2026-07-13 14:36] · GitHub Copilot · 文档同步与代码推送

**摘要**：将主线文档、ETL、SQL 和协作模板整理后提交，准备推送到远端分支

**变更文件**：

| 文件 | 变更类型 | 说明 |
|------|---------|------|
| `.github/copilot-instructions.md` | 修改 | 更新协作与硬约束说明 |
| `.github/skills/project-bootstrap-hefang/SKILL.md` | 新增 | 补充项目搭建模板与上下文交接模板 |
| `.github/skills/tableau-twb-compiler-hefang/SKILL.md` | 新增 | 补充 Tableau .twb 编译技能与资产 |
| `SQL/` | 新增/修改 | 补充 ODS/ADS/校验/回滚脚本 |
| `docs/` | 新增/修改 | 补充架构、运行手册、数据字典与交接材料 |
| `etl_*.py, run_*.py, tools/*.py` | 修改/新增 | 同步主线 ETL、调度与校验工具 |

**Copilot 接棒须知**：
- 已确认 .gitignore 覆盖 logs/、reports/、.tmp/、__pycache__/、.pytest_cache/ 等临时产物，未进入暂存区。
- 本次仅保留关键文档与代码，未纳入本机缓存、日志和其他运行输出。

**未完成项**：
- [ ] 如后续还有增量变更，需先更新 AGENT_HANDOFF 再继续推送
