...
| 状态筛选 | 没加STATUS=2 | 必须加！ |
| SKU筛选 | 未过滤M_PRODUCTALIAS_ID为空 | 必须加！ |
| 渠道筛选 | 未限制电商/云仓口径 | 必须加！ |
...
SELECT 
    r.BILLDATE AS 日期,
    s.NAME AS 渠道,
    SUM(CASE WHEN r.TOT_AMT_ACTUAL > 0 THEN ri.TOT_AMT_ACTUAL ELSE 0 END) AS ERP销售额
...
SELECT 
    ...
    (SELECT SUM(QTY) 
     ...
     WHERE fs2.M_PRODUCT_ID = p.ID
       AND fs2.ISACTIVE = 'Y'
       AND fs2.M_PRODUCTALIAS_ID IS NOT NULL
...
*文档版本: 2.1 | 更新日期: 2026-01-30 | 合并文档13、18*
